...
- 技术话题：未指向机器人旁观；直接提问时强制证据边界，不确定就索要日志/截图/官方状态。裸 URL 不因 `http` 被单独视为技术话题，会结合剩余文本和有界邻近上下文判断。
...
当 Planner 已明确判断要回复但没有发出 `reply`，或发出的 `reply` 目标被安全规则取消时，插件会在极小范围内兜底：
...
被安全规则取消的 `reply` 会视为存在回复意图；插件只会在同一安全窗口内回退到更早的有效机器人指向目标，不会重新扫描全历史，也不会绕过挖坟保护。
...
- 最多扫描最近 3 条；
- 超过约 5 分钟的候选不参与兜底；
- 最新消息明确 @/称呼机器人，或已由指向判断器判定为机器人时可以直接兜底；
- 历史候选必须有 Planner 的非否定回复意图，或存在被安全规则取消的有效 `reply`；畸形或幻觉 reply 不会让无关最新消息获得兜底；
